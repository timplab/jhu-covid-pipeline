...
#write.tree(jhu_tree,"data/AllClades_cat_wHP_wother_wo3000_woMD_woDC_jhu_rooted.nwk")
...
md_diff <- md_march - cumsum(jhhs_agg$positive_count)
...
df_a_long$variable <- factor(df_a_long$variable,
                             levels=c("md_diff", "jhhs_cumulative"))
levels(df_a_long$variable) <- c("Maryland Overall", "JHHS Overall")
...
fig1a <- ggplot(df_a_long, aes(x=date, y=value, fill=variable, color=variable)) +
  ...
  scale_y_continuous(limits=c(0,1750),breaks=seq(0,1750,250), expand = c(0,0)) +
  ...
  geom_hline(yintercept=seq(0,1500,250), color="white") +
...
ParwiseDistances <- read.delim("data/ParwiseDistances.txt")

ParwiseDistances$clade <- factor(ParwiseDistances$clade,levels = 
                                   c("Global 3K", "Global 1K", "WA","CA", "NY","ID","LA",
                                     "VA", "MD other", "JHHS-MD", "DC other","JHHS-DC"))

VP <- ggplot(ParwiseDistances, aes(x=clade, y=Dist, color = clade, fill = clade), scale = "area", trim = FALSE)
VP <- VP + geom_violin(draw_quantiles = c(0.5), adjust = 2) 
VP <- VP + scale_color_manual(values=c("#8D8F91", "#8D8F91", "#8D8F91","#8D8F91", "#8D8F91","#8D8F91","#8D8F91", "#67C4A2", "#1E72B3", "#1E72B3", "#F27045","#F27045"))
VP <- VP + scale_fill_manual(values=c("#FFFFFF", "#FFFFFF", "#FFFFFF","#FFFFFF", "#FFFFFF","#FFFFFF","#FFFFFF", "#FFFFFF", "#FFFFFF", "#1E72B3", "#FFFFFF","#F27045"))
